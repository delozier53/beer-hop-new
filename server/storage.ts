...
export interface IStorage {
  ...
  updateUserChallengeProgress(userId: string, challengeId: string, progress: number, data?: any): Promise<UserChallenge | undefined>;
  completeUserChallenge(userId: string, challengeId: string): Promise<UserChallenge | undefined>;
  ...
  updateChallengeLeaderboard(challengeId: string, userId: string, score: number): Promise<void>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getChallenges(): Promise<Challenge[]> {
    ...
    if (allChallenges.length === 0) {
      const defaultChallenges = await this.createDefaultChallenges();
  ...
  async updateUserChallengeProgress(userId: string, challengeId: string, progress: number, data?: any): Promise<UserChallenge | undefined> {
    const updates: Partial<UserChallenge> = { progress };
    if (data) {
      updates.data = data;
    }
  ...
  async completeUserChallenge(userId: string, challengeId: string): Promise<UserChallenge | undefined> {
    const [userChallenge] = await db
      .update(userChallenges)
      .set({
        status: 'completed',
  ...
  async getChallengeLeaderboard(challengeId: string): Promise<ChallengeLeaderboard[]> {
    return await db
      ...
      .orderBy(desc(challengeLeaderboard.score));
  }

  async updateChallengeLeaderboard(challengeId: string, userId: string, score: number): Promise<void> {
    await db
      ...
      .values({
        ...
        userId,
        score,
        rank: 0, // Will be calculated by ranking logic
        updatedAt: new Date()
      })
      .onConflictDoUpdate({
        target: [challengeLeaderboard.challengeId, challengeLeaderboard.userId],
        set: {
          score,
          updatedAt: new Date()
        }
    ...
    await this.updateChallengeRanks(challengeId);
  }

  // Helper methods
  private async createDefaultChallenges(): Promise<Challenge[]> {
    const defaultChallenges: InsertChallenge[] = [
      {
        title: "Brewery Explorer",
        description: "Visit 5 different breweries to complete this challenge",
        type: "variety",
        difficulty: "easy",
        targetValue: 5,
        timeLimit: 30, // 30 days
        points: 100,
        badge: "Brewery Explorer Badge",
        requirements: {}
      },
      {
        title: "Distance Warrior",
        description: "Travel 50 miles between brewery visits",
        type: "distance",
        difficulty: "medium",
        targetValue: 50,
        timeLimit: 60, // 60 days
        points: 200,
        badge: "Distance Warrior Badge",
        requirements: { minDistance: 1 }
      },
      {
        title: "Weekly Regular",
        description: "Check in to breweries 3 times in one week",
        type: "time",
        difficulty: "medium",
        targetValue: 3,
        timeLimit: 7, // 7 days
        points: 150,
        badge: "Weekly Regular Badge",
        requirements: { consecutiveDays: 7 }
      },
      {
        title: "Brewery Master",
        description: "Visit 25 unique breweries across Oklahoma",
        type: "variety",
        difficulty: "hard",
        targetValue: 25,
        points: 500,
        badge: "Brewery Master Badge",
        requirements: {}
      },
      {
        title: "Social Butterfly",
        description: "Share your brewery visits 10 times",
        type: "social",
        difficulty: "easy",
        targetValue: 10,
        timeLimit: 30, // 30 days
  ...
  private async awardChallengeCompletion(userId: string, challenge: Challenge): Promise<void> {
  ...
  private async updateChallengeRanks(challengeId: string): Promise<void> {
    ...
    for (let i = 0; i < leaderboard.length; i++) {
      await db
        ...
        .set({ rank: i + 1 })
        .where(eq(challengeLeaderboard.id, leaderboard[i].id));
    }
  }
}
